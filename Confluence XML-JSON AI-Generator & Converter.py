...
st.title("🔗 Confluence XML/JSON AI-Generator & Converter")
    ...
    if st.button('Generate Answer'):
        if query and context:
            response = ai_model.generate_content(
                f"Question: {query}\nContext: {context}\n"
                f"Answer the question with the exact code format from the context provided. "
                f"Just output the code without any explanation or extra text. Your output should look like raw code."
            )
            st.subheader("Answer (in code format):")
